---
title: (Restaurant Review) Report
create_date: 2024-07-24 14:07 - 2024-07-24 14:07
draft: true
---
...
`만족도`, `맛`, `서비스`, `가격` 카테고리로 분류하였다. `만족도`와 `맛`의 평가 리뷰수가 많았고, `서비스`, `가격`의 평가 리뷰수는 적었다. ([[Report#2. 각 카테고리별 총 긍/부정 개수 막대 그래프 시각화|각 카테고리별 총 긍/부정 평가 개수]])  

4개의 카테고리에 대한 긍/부정을 평가한 결과 ([[Report#3. 각 카테고리별 긍/부정 비율 원형 그래프 시각화|각 카에고리별 긍/부정 비율 시각화]])  

대부분 긍정의 리뷰를 남긴 것으로 확인되었다.  

가장 많은 긍정 리뷰를 가진 `만족도` 카테고리와 가장 많은 부정 리뷰를 가진 `가격` 카테고리를 더 상세하게 분석해보고자 했다.  

`만족도`와 `가격`의 긍/부정 리뷰글에서 빈도 높은 명사, 동사/형용사를 추출하여 그 중 명사들을 확인하여 어떤 키워드들이 긍/부정 평가에 기준이 되었는지 확인해보았다. ([[Report#4. 카테고리별 긍/부정 리뷰글 명사, 동사/형용사 추출 및 시각화|카테고리별 긍/부정 리뷰글 명사, 동사/형용사 추출및 시각화]])  
...
각 명사들의 언급된 수를 나타내면 이렇다. ([[Report#6. 빈도 높은 명사들의 총 긍/부정 개수| 빈도 높은 명사들의 총 긍/부정 개수]])

각 리뷰글에서 음식점 특성상 음식에 대한 언급이 많았고 가격과 친절이 그 뒤를 이었다.

각 명사들의 긍/부정 비율을 나타내면 이렇다. ([[Report#7. 빈도 높은 명사들의 긍/부정 시각화|빈도 높은 명사들의 긍/부정 시각화]])
...
마지막으로 `가격`에 대한 평가로는 대부분의 사람들이 "비싸다"와 같은 부정적인 평가를 하였지만 비싸지만 가격이외에서 만족하여 종합적으론 가격에 대해선 부정적이지만 다른면에서 긍정적인 평가를 보여 전체적으로 긍정적인 평가를 가졌다.  
